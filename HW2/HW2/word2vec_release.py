...
def loadData(filename):
    global uniqueWords, wordcodes, wordcounts, UNKword
    override = True
    if override:
...
#     #... after filling in fullrec_filtered, replace the original fullrec with this one.
    for word in fullrec:
         if (origcounts[word] < min_count) & (word not in test_word_new):
                UNKword.append(betterUNK(word))
    fullrec_filtered = [betterUNK(word) if (origcounts[word] < min_count) & (word not in test_word_new)  else word for word in fullrec]
    fullrec_filtered2 = []
